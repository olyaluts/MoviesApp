...
let container = Container { container in
    ...
    container.register(MovieService.self) { resolver in
        guard let networkClient = resolver.resolve(NetworkProtocol.self) else {
            fatalError("Failed to init NetworkProtocol")
        }
...
final class MoviesCoorinator {
    ...
    private func createLandingViewController() -> UIViewController {
        guard let movieService = container.resolve(MovieService.self) else {
            fatalError("Failed to create MovieService")
        }
        let context = MovieViewModelImpl.ServiceContext(
        ...
        let viewModel = MovieViewModelImpl(
            context: context,
            handlers: handlers)
